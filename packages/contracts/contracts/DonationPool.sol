...
contract DonationPool is Ownable, ReentrancyGuard {
    ...
    function donate(uint256 filmId) external payable nonReentrant {
        ...
        // Pull payment pattern is better, but here we use call for security
        for (uint256 i = 0; i < film.creators.length; i++) {
            uint256 creatorAmount = (creatorTotal * film.shares[i]) / BPS_DENOMINATOR;
            (bool success, ) = film.creators[i].call{value: creatorAmount}("");
            require(success, "Creator payment failed");
        }

        (bool fSuccess, ) = creatorFund.call{value: fundAmount}("");
        require(fSuccess, "Fund payment failed");

        (bool tSuccess, ) = treasury.call{value: platformAmount}("");
        require(tSuccess, "Treasury payment failed");
