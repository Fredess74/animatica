...
contract DonationPool is Ownable, ReentrancyGuard {
    ...
    function donate(uint256 filmId) external payable nonReentrant {
        ...
        // Allocate creator shares for withdrawal
        ...
        // Send to system funds immediately using call to avoid gas limitations
        (bool fundSuccess, ) = payable(creatorFund).call{value: fundAmount}("");
        require(fundSuccess, "CreatorFund transfer failed");

        (bool treasurySuccess, ) = payable(treasury).call{value: platformAmount}("");
