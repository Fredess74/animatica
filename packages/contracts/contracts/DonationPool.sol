...
contract DonationPool is Ownable, ReentrancyGuard {
    ...
    function donate(uint256 filmId) external payable nonReentrant {
        ...
        // Pull Pattern: Add to pending withdrawals instead of pushing
        ...
        // Use .call for treasury and fund to avoid gas limit issues
        (bool successFund, ) = payable(creatorFund).call{value: fundAmount}("");
        require(successFund, "Fund transfer failed");

        (bool successTreasury, ) = payable(treasury).call{value: platformAmount}("");
