...
contract DonationPool is Ownable, ReentrancyGuard {
    ...
    function donate(uint256 filmId) external payable nonReentrant {
        ...
        for (uint256 i = 0; i < film.creators.length; i++) {
            ...
            (bool s, ) = film.creators[i].call{value: creatorAmount}("");
            require(s, "Creator transfer failed");
        }

        (bool s1, ) = creatorFund.call{value: fundAmount}("");
        require(s1, "Fund transfer failed");

        (bool s2, ) = treasury.call{value: platformAmount}("");
