...
const ASSET_CATEGORIES: Record<string, AssetItem[]> = {
    Primitives: [
        ...
        { name: 'Sphere', icon: '🔮', type: 'primitive', defaults: { shape: 'sphere', color: '#4ADE80' } },
        { name: 'Cylinder', icon: '🛢️', type: 'primitive', defaults: { shape: 'cylinder', color: '#86EFAC' } },
        { name: 'Cone', icon: '🔺', type: 'primitive', defaults: { shape: 'cone', color: '#16A34A' } },
        { name: 'Torus', icon: '🍩', type: 'primitive', defaults: { shape: 'torus', color: '#0D7A48' } },
        { name: 'Plane', icon: '⬜', type: 'primitive', defaults: { shape: 'plane', color: '#F5F5F0' } },
    ],
    Lights: [
        { name: 'Point Light', icon: '💡', type: 'light', defaults: { lightType: 'point', intensity: 1 } },
        { name: 'Spot Light', icon: '🔦', type: 'light', defaults: { lightType: 'spot', intensity: 1 } },
        { name: 'Directional', icon: '☀️', type: 'light', defaults: { lightType: 'directional', intensity: 0.5 } },
    ],
    Cameras: [
        { name: 'Camera', icon: '📷', type: 'camera', defaults: { fov: 75, near: 0.1, far: 1000 } },
    ],
    Characters: [
        { name: 'Humanoid', icon: '🧑', type: 'character', defaults: { model: 'default' } },
    ],
};
            ...
            {Object.entries(ASSET_CATEGORIES).map(([category, items]) => (
                <div key={category} className="asset-category">
                    ...
                    >
                        ...
                        <span>{category}</span>
                        <span className="asset-category__count" aria-label={`${items.length} items`}>{items.length}</span>
                    </button>
                        ...
                        <div className="asset-category__items" id={`category-${category}`}>
                            {items.map((item) => (
                                <button
                                    ...
                                    title={`Add ${item.name}`}
                                    aria-label={`Add ${item.name} ${item.type}`}
                                >
