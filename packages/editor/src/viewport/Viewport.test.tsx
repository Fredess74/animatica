...
vi.mock('@Animatica/engine', () => ({
  // SceneManager is used in SceneManager.tsx (engine) but also imported in editor's Viewport.tsx
  SceneManager: () => <div data-testid="scene-manager" />,
  // Export individual renderers because SceneRenderer.tsx imports them
...
// Mock Viewport components that use internal R3F state or hooks
vi.mock('./SceneRenderer', () => ({
    SceneRenderer: () => <div data-testid="scene-renderer" />
}))

// Mock ViewportOverlay to avoid its dependency on useSceneStore find
...
describe('Viewport', () => {
  ...
  it('renders gizmo when object is found', async () => {
    ...
    // The gizmo has a 50ms timeout in ViewportGizmo.tsx
    await waitFor(() => {
