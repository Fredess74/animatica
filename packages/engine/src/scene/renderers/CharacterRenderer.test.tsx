...
describe('CharacterRenderer', () => {
  ...
  it('renders a group containing capsule mesh with correct transform', () => {
    ...
    // Rig should be present via primitive
    const primitive = children.find(c => c.type === 'primitive')
    expect(primitive).toBeDefined()

    // Check for rig root object prop if needed, but the current implementation
    // uses procedural rig which is mocked or created by createProceduralHumanoid
  })
  ...
  it('renders selection ring when selected', () => {
    ...
    // Check for the ring mesh (it's the second child if it exists)
    const ringMesh = children.find(c => c.type === 'mesh')
    expect(ringMesh).toBeDefined()

    const ringProps = (ringMesh as any).props
