...
export const CharacterRenderer = React.memo(forwardRef<THREE.Group, CharacterRendererProps>(({
  actor,
  isSelected = false,
  onClick,
}, ref) => {
  ...
  // Expose groupRef to parent if needed
  ...
  useFrame((_state, delta) => {
    // If not visible, we can skip animation updates to save resources
    ...
    if (eyeControllerRef.current && faceMorphRef.current) {
      const headPos = groupRef.current
        ? TEMP_VECTOR.setFromMatrixPosition(groupRef.current.matrixWorld)
        : undefined
  ...
  // Early return after all hook calls to satisfy Rules of Hooks
