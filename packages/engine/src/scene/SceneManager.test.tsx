...
import { useSceneStore, useActorList, useEnvironment, useTimeline, useCurrentTime } from '../store/sceneStore'
...
describe('SceneManager', () => {
  beforeEach(() => {
    ...
    ;(useSceneStore as any).mockImplementation((selector: any) => selector(defaultState))
    ;(useActorList as any).mockReturnValue(defaultState.actors)
    ;(useEnvironment as any).mockReturnValue(defaultState.environment)
    ;(useTimeline as any).mockReturnValue(defaultState.timeline)
    ;(useCurrentTime as any).mockReturnValue(defaultState.playback.currentTime)
  })

  it('renders primitive actors', () => {
    const actors = [{ id: '1', type: 'primitive' }]
    ;(useActorList as any).mockReturnValue(actors)
