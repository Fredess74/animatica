...
vi.mock('../store/sceneStore', () => ({
  ...
  useTimelineValue: vi.fn(),
  useCurrentTime: vi.fn(),
  useActorIds: vi.fn(),
}))

import { useEnvironmentValue, useTimelineValue, useCurrentTime, useActorIds } from '../store/sceneStore'
...
describe('SceneManager', () => {
  beforeEach(() => {
    ...
    ;(useEnvironmentValue as any).mockImplementation((selector: any) => selector(defaultEnv))
    ;(useTimelineValue as any).mockImplementation((selector: any) => {
      const timeline = { cameraTrack: [], animationTracks: [] }
  ...
  it('renders primitive actors', () => {
    ...
    ;(useSceneStore as any).mockImplementation((selector: any) => selector({ actors }))
