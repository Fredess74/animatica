...
import * as SceneStoreHooks from '../store/sceneStore'

// Mock the store hooks individually
...
describe('SceneManager', () => {
  beforeEach(() => {
    ...
    ;(SceneStoreHooks.useActorList as any).mockReturnValue([])
    ;(SceneStoreHooks.useAmbientLight as any).mockReturnValue({ intensity: 0.5, color: '#ffffff' })
    ;(SceneStoreHooks.useSun as any).mockReturnValue({ position: [10, 10, 10], intensity: 1, color: '#ffffff' })
    ;(SceneStoreHooks.useSkyColor as any).mockReturnValue('#87CEEB')
    ;(SceneStoreHooks.useFog as any).mockReturnValue(undefined)
    ;(SceneStoreHooks.useTimeline as any).mockReturnValue({ cameraTrack: [], animationTracks: [] })
    ;(SceneStoreHooks.useCurrentTime as any).mockReturnValue(0)
  })

  it('renders primitive actors', () => {
    const actors = [{ id: '1', type: 'primitive' }]
    ;(SceneStoreHooks.useActorList as any).mockReturnValue(actors)
