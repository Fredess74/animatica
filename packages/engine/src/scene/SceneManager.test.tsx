...
describe('SceneManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(useAmbientLight).mockReturnValue({ intensity: 0.5, color: '#ffffff' })
    vi.mocked(useSun).mockReturnValue({ position: [10, 10, 10], intensity: 1, color: '#ffffff' })
    vi.mocked(useSkyColor).mockReturnValue('#87CEEB')
    vi.mocked(useFog).mockReturnValue(undefined)
  ...
  it('renders primitive actors', () => {
    const actors = [{ id: '1', type: 'primitive' }] as any
    vi.mocked(useActorList).mockReturnValue(actors)
  ...
  it('renders light actors', () => {
    const actors = [{ id: '2', type: 'light' }] as any
    vi.mocked(useActorList).mockReturnValue(actors)
  ...
  it('renders camera actors', () => {
    const actors = [{ id: '3', type: 'camera' }] as any
    vi.mocked(useActorList).mockReturnValue(actors)
  ...
  it('renders character actors', () => {
    const actors = [{ id: '4', type: 'character' }] as any
    vi.mocked(useActorList).mockReturnValue(actors)
  ...
  it('renders speaker actors', () => {
    const actors = [{ id: '5', type: 'speaker' }] as any
    vi.mocked(useActorList).mockReturnValue(actors)
