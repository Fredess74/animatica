...
describe('Engine Benchmarks', () => {
    ...
    describe('Store Performance', () => {
        ...
        it('Store Actor CRUD Throughput (1k actors)', () => {
            ...
            measure('Store Add Actor (1k ops)', 1000, (i) => {
                getState().addActor({
                    id: `bench-${i}`,
                    name: `Actor ${i}`,
                    type: 'primitive',
            ...
            measure('Store Update Actor (1k ops)', 1000, (i) => {
                getState().updateActor(`bench-${i}`, { visible: false });
            });

            measure('Store Remove Actor (1k ops)', 1000, (i) => {
                getState().removeActor(`bench-${i}`);
            }, () => {
                // Ensure store is prepopulated with actors for the timed removal run
                for (let i = 0; i < 1000; i++) {
                    if (!getState().actors.find(a => a.id === `bench-${i}`)) {
                         getState().addActor({
                            id: `bench-${i}`,
                            name: `Actor ${i}`,
                            type: 'primitive',
