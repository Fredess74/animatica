...
- **Phase 2: Characters** & **Phase 3: Editor UI** advancements:
    - `BoneController` for programmatic character posing and skeleton manipulation.
    - Integration of BoneController into `CharacterRenderer`.
    - 2D Storyboard mode for alternative scene visualization.
    - Optimized Zustand store state management with improved selectors and shallow comparisons.
    - Enhanced character animation coverage and regression testing.

### Changed

- Refactored `CharacterRenderer` to improve visibility handling and ref management.
- Optimized bundle sizes for `@Animatica/engine` and `@Animatica/editor`.
- Updated license audit and project metrics.

### Fixed

- Resolved regressions in `CharacterRenderer.test.tsx`.
- Improved stability of Viewport rendering in the editor.
