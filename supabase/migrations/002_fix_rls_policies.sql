...
-- This migration drops the insecure UPDATE policies and recreates them with WITH CHECK clauses
-- to ensure sensitive system-managed columns cannot be modified by users.
...
WITH CHECK (
  ...
  (
    ...
    NEW.creator_weight IS NOT DISTINCT FROM OLD.creator_weight AND
...
WITH CHECK (
  ...
  (
    ...
    NEW.chain_film_id IS NOT DISTINCT FROM OLD.chain_film_id AND
    NEW.created_at IS NOT DISTINCT FROM OLD.created_at
  )
);
...
WITH CHECK (
  ...
  (
    ...
    NEW.is_approved IS NOT DISTINCT FROM OLD.is_approved AND
    NEW.created_at IS NOT DISTINCT FROM OLD.created_at
  )
);
...
WITH CHECK (
  ...
  (
    NEW.owner_id IS NOT DISTINCT FROM OLD.owner_id AND
    NEW.subscriber_count IS NOT DISTINCT FROM OLD.subscriber_count AND
...
WITH CHECK (
  ...
  (
    ...
    NEW.like_count IS NOT DISTINCT FROM OLD.like_count AND
    NEW.reply_count IS NOT DISTINCT FROM OLD.reply_count AND
    NEW.created_at IS NOT DISTINCT FROM OLD.created_at
  )
);
